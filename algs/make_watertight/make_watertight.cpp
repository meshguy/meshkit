...
MBErrorCode create_arc_pair(  const double FACET_TOL,
  ...
  for(unsigned i=0; i<curve_sets.size(); ++i) {
    ...
    if(front_endpt==temp_curve.front() && 1!=sense) {
    ...
    if(front_endpt==temp_curve.back() && 0!=sense) {
...
MBErrorCode seal_loop( bool debug,
  ...
  /* Find the closest skin pt to the first curve's front endpt.
	...
	//        if(gen::error(curve.front()==curve.back(),"degenerate curve endpts")) return MB_FAILURE;
      ...
      MBEntityHandle curve_endpt = (0==sense) ? curve.front() : curve.back();
...
MBErrorCode get_geom_size_after_sealing( const MBRange geom_sets[], 
  ...
  int main(int argc, char **argv) {
    ...
    std::string output_file = "watertight_out.h5m";
    std::string input_file;

    ProgOptions po("make_watertight: Create watertight version of faceted geometry");

    po.addOpt<std::string>(",o", "Set name of output file.  Default: "+ output_file, &output_file );
    po.addRequiredArg( "input_file", "Geometry input file", &input_file );
    ...
    if(gen::error(MB_SUCCESS!=rval,"failed to load_file")) return rval;      
