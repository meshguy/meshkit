...
namespace cleanup {
  ...
  MBErrorCode remove_obb_tree() {
    ...
    MBTag obbTag;
    //tag_get_handle(name, size, type, retval, flags, def_value);
    result = MBI()->tag_get_handle( "OBB_TREE", 1, MB_TYPE_HANDLE, obbTag,
    ...
    result = MBI()->tag_get_handle( "OBB", 1,
     				  MB_TYPE_DOUBLE, rootTag, moab::MB_TAG_SPARSE|moab::MB_TAG_CREAT);
  ...
  MBErrorCode delete_small_edges(const MBRange &surfaces, const double FACET_TOL) {
	...
	*/
	...
	std::cout << "next iter=" << *j << std::endl;        
        ...
        result = tool.find_skin( 0, new_tris, 1, new_skin_edges, false );
        assert(MB_SUCCESS == result);
