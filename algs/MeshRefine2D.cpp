...
MeshRefine2D:: face_centroid( iBase_EntityHandle facehandle) const
{
  ...
  int numNodes = facenodes.size(); assert( numNodes > 2 );
...
MeshRefine2D:: face_aspect_ratio( iBase_EntityHandle facehandle) const
{
  ...
  double minlen = *min_element( elen.begin(), elen.end() );
  double maxlen = *max_element( elen.begin(), elen.end() );
...
void LongestEdgeRefine2D :: atomicOp( const iBase_EntityHandle &oldface)
{
  int err;
...
int LongestEdgeRefine2D::execute() 
{
  ...
  for( int i = 0; i < faces.size(); i++) {
	double ratio  = face_aspect_ratio( faces[i] );
        if( ratio < cutOffAspectRatio) atomicOp( faces[i] );
  }
...
int LongestEdgeRefine2D :: finalize() 
{
...
int ConsistencyRefine2D :: finalize()
{
    return 0;
}
...
void ConsistencyRefine2D :: atomicOp(const iBase_EntityHandle &oldface)
{
    int err, val;
    iMesh_getIntData(mesh, oldface, remove_tag, &val, &err);

    if( err == 0 && val == 1 ) return;
...
int ObtuseRefine2D :: atomicOp( const iBase_EntityHandle &face)
{
/*
    iBase_EntityHandle v0, v1, v2, vmid;

    double angle;
    for( int i = 0; i < 3; i++) {
         v0 = face->getVertex((i+0)%3); assert(v0);
         v1 = face->getVertex((i+1)%3); assert(v1);
         v2 = face->getVertex((i+2)%3); assert(v2);
         angle = MeshUtil::get_degree_angle(v0,v1, v2);
...
/*
     ...
     initialize();
     FaceMesh::const_iterator it;
     FOR_EACH(mesh, it) atomicOp(*it);
    
     ConsistencyRefine2D refine(facemesh, edgemap);
     refine.execute();

     insertedFaces = refine.getNewFaces();
