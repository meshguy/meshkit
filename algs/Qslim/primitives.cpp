...
void filterValid(std::vector<MBEntityHandle> & input, std::vector<
		MBEntityHandle> & filtered) {
	for (int i = 0; i < input.size(); i++) {
		MBEntityHandle eh = input[i];
		if (ehIsValid(eh))
			filtered.push_back(eh);
	}
	return;
}
...
MBErrorCode contractionRegion(MBInterface * mb, MBEntityHandle v1,
	...
	std::vector<MBEntityHandle> changedTotal;
	MBErrorCode rval = mb->get_adjacencies(vlist, 2, 2, false, changedTotal,
			MBInterface::UNION);
	// see if they are valid
	filterValid(changedTotal, changed);
	return rval;
}
...
int classifyVertex(MBInterface * mb, MBEntityHandle v1) {
	...
	// border only (corner)
	// to do
	std::vector<MBEntityHandle> adjEdgesTotal;
	MBErrorCode rval = mb->get_adjacencies(&v1, 1, 1, false, adjEdgesTotal,
			MBInterface::UNION);
	if (MB_SUCCESS != rval)
		return 0; // interior??
	// see if they are valid
	std::vector<MBEntityHandle> adjEdges;
	filterValid(adjEdgesTotal, adjEdges);
	int nBorder = 0;
	for (int i = 0; i < adjEdges.size(); i++) {
		MBEntityHandle edg = adjEdges[i];
		std::vector<MBEntityHandle> adjFacesTotal;
		rval = mb->get_adjacencies(&edg, 1, 2, false, adjFacesTotal,
				MBInterface::UNION);
		std::vector<MBEntityHandle> adjFaces;
		filterValid(adjFacesTotal, adjFaces);
		if (adjFaces.size() == 1)
			nBorder++;
	}
	if (nBorder == 0)
...
MBErrorCode contract(MBInterface * mb, MBEntityHandle v0, MBEntityHandle v1,
	...
	// find also all edges that are affected
	MBEntityHandle vlist[2];
	vlist[0] = v0;
	vlist[1] = v1;
	// it makes more sense to use vector, than range
	//  the entities could be very disjoint at some point
	// MBRange adj_ents;
	std::vector<MBEntityHandle> edgesTotal;
	MBErrorCode rval = mb->get_adjacencies(vlist, 2, 1, false, edgesTotal,
			MBInterface::UNION);
	std::vector<MBEntityHandle> edges;
	filterValid(edgesTotal, edges);
	...
	//mb->set_coords(&v1, 1, newCoords); we do not have to, actually
	...
	if (foundEdge) {
		...
		std::vector<MBEntityHandle> trisTotal;
		rval = mb->get_adjacencies(&ev0v1, 1, 2, false, trisTotal,
				MBInterface::UNION);
		// find all edges that will be merged ( xv0, xv1, etc)
		std::vector<MBEntityHandle> tris;
		filterValid(trisTotal, tris);
		if (opts.logfile && opts.selected_output & OUTPUT_CONTRACTIONS) {
			*opts.logfile << "Triangles adjacent to found edge:" << tris.size()
					<< ":";
		...
		for (int i = 0; i < tris.size(); i++) {
			...
			std::vector<MBEntityHandle> localEdgesTotal;
			rval = mb->get_adjacencies(&triangleThatCollapses, 1, 1, false,
					localEdgesTotal, MBInterface::UNION);
			std::vector<MBEntityHandle> localEdges;
			filterValid(localEdgesTotal, localEdges);
			if (opts.logfile && opts.selected_output & OUTPUT_CONTRACTIONS) {
				*opts.logfile << "Triangle " << mb->id_from_handle(
				...
				for (int i = 0; i < localEdges.size(); i++)
					*opts.logfile << " " << mb->id_from_handle(localEdges[i]);
				*opts.logfile << std::endl;
			}
		...
		rval = mb->tag_set_data(validTag, &(tris[0]), tris.size(), &invalid);
		if (opts.logfile && opts.selected_output & OUTPUT_CONTRACTIONS) {
				...
				if (conn3[j] == v1) {
					...
					if (opts.logfile && opts.selected_output
						...
						*opts.logfile << "Triangle t" << mb->id_from_handle(tr)
								<< " total: " << localEdgesTotal.size();
						for (int j = 0; j < localEdgesTotal.size(); j++)
							*opts.logfile << " e" << mb->id_from_handle(
									localEdgesTotal[j]);
						*opts.logfile << std::endl;
						*opts.logfile << "  filtered : " << localEdges.size();
					...
					if (opts.logfile && opts.selected_output
						...
						*opts.logfile << "replace connectivity t"
								...
								<< mb->id_from_handle(conn3[2]) << "  to: v";
					}
					rval = mb->set_connectivity(tr, connNew, 3);
					if (opts.logfile && opts.selected_output
							& OUTPUT_CONTRACTIONS) {
						*opts.logfile << mb->id_from_handle(connNew[0]) << " v"
								<< mb->id_from_handle(connNew[1]) << " v"
								<< mb->id_from_handle(connNew[2]) << std::endl;
					}
		...
		rval = mb->tag_set_data(validTag, &edgesWithV1[0], edgesWithV1.size(),
				&invalid);
				...
				if (conn[j] == v1) {
					...
					if (opts.logfile && opts.selected_output
						...
						*opts.logfile << mb->id_from_handle(connNew[0]) << " "
								<< mb->id_from_handle(connNew[1]) << std::endl;
					}
				}
			}
		}
		...
		for (int i = 0; i < trisChanged.size(); i++) {
			...
			rval = mb->get_adjacencies(&tr, 1, 1, false, localEdgesTotal,
					MBInterface::UNION);
			std::vector<MBEntityHandle> localEdges;
			filterValid(localEdgesTotal, localEdges);
			if (opts.logfile && opts.selected_output & OUTPUT_CONTRACTIONS) {
				...
				*opts.logfile << "  filtered : " << localEdges.size();
				for (int j = 0; j < localEdges.size(); j++)
					*opts.logfile << " e" << mb->id_from_handle(localEdges[j]);
				*opts.logfile << std::endl;
			}
			assert(localEdges.size()==3);
		}

		filterValid(adj_entities, changed);
		if (opts.logfile && opts.selected_output & OUTPUT_CONTRACTIONS) {
	...
	} else // this should appear only when proximity limit > 0
	{
		...
		rval
				= mb->get_adjacencies(&v0, 1, 1, false, edges0,
						MBInterface::UNION);

		// get edges connected to vertex v1
		std::vector<MBEntityHandle> edges1;
		rval
				= mb->get_adjacencies(&v1, 1, 1, false, edges1,
						MBInterface::UNION);
