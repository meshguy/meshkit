...
////
    ...
    for (int i=0; i<edges.size(); i++)
    {
       ...
       if (opts.logfile && opts.selected_output&OUTPUT_CONTRACTIONS )
		 *opts.logfile << "Edge: " << mb->id_from_handle(e)
		  << " nnodes:" << nnodes
		  << " vertices:" << mb->id_from_handle(conn2[0]) << " "
...
//
    ...
    if (foundEdge)
    {
	  ...
	  MBEntityHandle triangleThatCollapses= tris[i];
          ...
          if (opts.logfile && opts.selected_output&OUTPUT_CONTRACTIONS )
		  {
			   *opts.logfile << "Triangle " <<  mb->id_from_handle(triangleThatCollapses) <<
       ...
       for (int j=0; j<edgePairsToMerge.size(); j+=2)
       {
           // will also delete edges that contained v1 before
             if (opts.logfile && opts.selected_output&OUTPUT_CONTRACTIONS )
	       	   *opts.logfile << "Edges merged:" << mb->id_from_handle(edgePairsToMerge[j])
	       	       	  << " " << mb->id_from_handle(edgePairsToMerge[j+1])<< std::endl;
       ...
       if (opts.logfile && opts.selected_output&OUTPUT_CONTRACTIONS )
	  *opts.logfile << "Edge invalidated" << mb->id_from_handle(ev0v1) << std::endl;
