...
class pair_info : public Heapable
{
   public:
      ...
      pair_info ( MBEntityHandle a, MBEntityHandle b ) { v0=a; v1=b; cost=HUGE; }

      bool isValid() { return ehIsValid(v0) && ehIsValid(v1); }//  :v0->isValid() && v1->isValid(); }
};
...
static
      pair_info *new_pair (MBEntityHandle v0, MBEntityHandle v1)//  Vertex *v0, Vertex *v1 )
{
          ...
          MBEntityHandle v2, /*Vertex *v1, Vertex *v2,*/ Vec3& vnew )
{
   ...
   if (rval!=MB_SUCCESS)
   {
	   std::cout << "error in getting adjacency information vs: "<< mb->id_from_handle(v1) << " "
      ...
      void compute_pair_info ( pair_info *pair /* Model * pM0,*/)
{
  ...
  // Vertex *v1 = pair->v1;
   ...
   if (opts.logfile && opts.selected_output&OUTPUT_COST)
   {
	   ...
	   *opts.logfile << " i/u pair (" << uniqID(pair->v0) << "," << uniqID(pair->v1) <<
			   ")=" << pair->cost <<  "  min : (" << uniqID(pairTop->v0) << "," <<
      ...
      void do_contract ( pair_info *pair )
{
   ...
   std::vector<MBEntityHandle> changed;
   contract (mb, v0, v1, vnew, changed );
   ...
   for ( i=0; i<v1_info.pairs.length(); i++ )
   {
      ...
      else std::cerr << "YOW!  This is a bogus pair." << std::endl;
...
void decimate_contract ( )
{
   ...
   if ( opts.logfile && ( opts.selected_output&OUTPUT_COST ) )
      *opts.logfile << "#$cost " << validFaceCount << " "
            << pair->cost << std::endl;
...
int  QslimDecimation::decimate(QslimOptions & iOpts)
{
   ...
   clock_t currentTime=init_time;
    ...
    for (MBRange::iterator rit =verts.begin(); rit!=verts.end(); rit++)
    {
    	...
    	if (ehIsValid(v))
    		continue;
    	invalidVerts.push_back(v);
    }
    // now delete all the invalid vertices
...
int  QslimDecimation::Init ()
{
  ...
  if (MB_SUCCESS != rval)
   ...
   rval = mb->tag_create("valid", sizeof(int), MB_TAG_DENSE, validTag, &def_data);
    if (MB_SUCCESS != rval)
      return 1;
  ...
  for (MBRange::iterator it=verts.begin(); it!=verts.end(); it++, i++)
  {
     ...
     rval = mb->tag_set_data(validTag, &v, 1, &def_data);
  }
  MBRange::iterator it;
  ...
  std::cout << "  Decimate:  Distributing shape constraints." << std::endl;
   ...
   if ( opts.pair_selection_tolerance<0 )
   {
	  ...
	  std::cout << "  Decimate:  Auto-limiting at 5% of model radius." << std::endl;
   }
   proximity_limit = opts.pair_selection_tolerance * opts.pair_selection_tolerance;
   if ( proximity_limit > 0 )
   {
	   std::cout << "  Decimate:  Collecting pairs [limit="<<opts.pair_selection_tolerance<<"]." << std::endl;
	  ...
	  if (rval!=MB_SUCCESS)
	 {
	  ...
	  for (it = verts.begin(); it!=verts.end(); it++)
	  {
         ...
         kd.leaves_within_distance(tree_root, coords, opts.pair_selection_tolerance, leaves);
         // add to the list of close vertices
         for (j=0; j<leaves.size(); j++)
         {
        	 rval = mb->get_entities_by_type(leaves[j], MBVERTEX, closeVertices);// add to the list
         }

         for ( MBRange::iterator it2=closeVertices.begin(); it2!=closeVertices.end(); it2++)
         {
   ...
   std::cout << "  Decimate:  Designated " << pair_count << " pairs." << std::endl;
