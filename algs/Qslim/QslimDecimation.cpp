...
MBTag validTag; // this will be used to mark validity of EH
...
// the edges are deleted, though, and triangles: maybe we will delay that too??
...
int QslimDecimation::decimate(QslimOptions & iOpts) {
	...
	// start with triangles, then edges, then vertices
...
int QslimDecimation::Init() {
	...
	if (MB_SUCCESS != rval)
		return 1;
	unsigned char def_data_char = 1;// valid by default
	rval = mb->tag_create("valid", 1, MB_TAG_BIT, validTag, &def_data_char);
	if (MB_SUCCESS != rval)
		return 1;
	...
	i = 0; // for index
	MBRange::iterator it;
	for (it = verts.begin(); it != verts.end(); it++, i++) {
		...
		rval = mb->tag_set_data(validTag, &v, 1, &def_data_char);
	}

	// set valid tag for edges and triangles, too
	for (it = edgs.begin(); it != edgs.end(); it++) {
		// this index i will be the index in the vinfo array
		MBEntityHandle e = *it;
		// is it really necessary if we put the default value as 1 anyway
		rval = mb->tag_set_data(validTag, &e, 1, &def_data_char);
	}
	for (it = triangles.begin(); it != triangles.end(); it++) {
		// this index i will be the index in the vinfo array
		MBEntityHandle t = *it;
		// is it really necessary if we put the default value as 1 anyway
		rval = mb->tag_set_data(validTag, &t, 1, &def_data_char);
	}
	...
	if (opts.logfile && opts.selected_output & OUTPUT_MODEL_DEFN) {
		...
		for (it = triangles.begin(); it != triangles.end(); it++) {
			...
			MBEntityHandle t = *it;
			// is it really necessary if we put the default value as 1 anyway
			std::vector<MBEntityHandle> edgesV;
			mb->get_adjacencies(&t, 1, 1, false, edgesV, MBInterface::UNION);
			const MBEntityHandle * conn;
			int num_nodes;
			rval = mb->get_connectivity(t, conn, num_nodes);

			*opts.logfile << " t" << mb->id_from_handle(t) << " v"
					<< mb->id_from_handle(conn[0]) << " v"
					<< mb->id_from_handle(conn[1]) << " v"
					<< mb->id_from_handle(conn[2]);
			for (int j = 0; j < 3; j++) {
				rval = mb->get_connectivity(edgesV[j], conn, num_nodes);
				*opts.logfile << "  e" << mb->id_from_handle(edgesV[j]) << "(v"
						<< mb->id_from_handle(conn[0]) << ",v"
						<< mb->id_from_handle(conn[1]) << ") ";
			}
