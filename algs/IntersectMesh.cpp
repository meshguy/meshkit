...
MBEntityHandle RedBoundary(0); // a triangle that is marking the boundary, null handle
MBEntityHandle BlueBoundary(0); // a triangle marking the boundary ? null handle
...
int getXYcoordsAndNodeHandles(MBInterface * MB, MBEntityHandle tr, double * coords,
		const MBEntityHandle *& conn) {
	//const MBEntityHandle * conn;
	int num_nodes;
	MBErrorCode rval = MB->get_connectivity(tr, conn, num_nodes);
...
void createTags(MBInterface * m1, MBInterface * m2, MBInterface * m3) {
	...
	rval = m2->tag_create( "redNodeTag", sizeof(MBEntityHandle),
	                                     MB_TAG_DENSE,
	                                     MB_TYPE_HANDLE,
	                                     RedNodeTag,
	                                     &def_val );
	if (MB_SUCCESS != rval)
		return;
	rval = m1->tag_create("blueNodeTag", sizeof(MBEntityHandle),
            MB_TAG_DENSE,
            MB_TYPE_HANDLE,
            BlueNodeTag,
            &def_val );
	...
	MBRange redEdges;
	rval = m2->get_entities_by_type(mbs2,
            MBTRI,
            redTriangles,
            false);
	if (MB_SUCCESS != rval)
	...
	rval = m2 ->get_adjacencies(redTriangles, 1, true, redEdges, MBInterface::UNION);
		...
		for (int j = 0; j < 3; j++) {
			...
			if (delta != 0.) {
				...
				if (0 <= alfa && alfa <= 1. && 0 <= beta && beta <= 1.) {
					...
					for (int k = 0; k < 2; k++) {
						points[2 * nPoints + k] = red[2 * i + k] + alfa
								* (red[2 * iPlus1 + k] - red[2 * i + k]);
					}
...
int borderPointsOfXinY2(double * X, double * Y, double * P) {
	...
	for (int i = 0; i < 3; i++) {
		...
		for (int j = 0; j < 3; j++) {
			...
			double area2 = (B[0] - A[0]) * (C[1] - A[1]) - (C[0] - A[0])
					* (B[1] - A[1]);
			if (area2 < 0.) {
				inside = 0;
				break;
...
MBErrorCode GetOrderedNeighbors(MBInterface * MB, MBEntityHandle tri, MBEntityHandle neighbors[3])
{
...
void CreateANewNode(double * coord, MBEntityHandle & outNode){
	// this new node will have coordinates
...
int IntersectMesh::compute(iBase_EntityHandle t1, iBase_EntityHandle t2,
	...
	while (!blueQueue.empty()) {
		MBEntityHandle n[3]; // flags for the side : 0 means a red triangle not found
		for (k = 0; k < 3; k++)
			n[k] = 0; // a paired red not found yet for the neighbors of blue
			...
			if (status==0 && n[j] > 0) // not treated yet and marked as a neighbor
			{
				...
				if (dbg)
					std::cout << "new triangles pushed: blue, red:"
							<< mb1->id_from_handle(blueNeigh ) << " " << mb1->id_from_handle(n[j] ) << std::endl;
