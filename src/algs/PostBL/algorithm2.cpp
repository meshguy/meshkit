...
// Inputs:
// d-dims sets gettign BL's.
// tag[+val] marking d-1 sets
// designation of B/P/NP for d-1 sets and setting default values



// generate new nodes on front

// replace d-1, d-2 ents on skin

// change coords of original d cells

// check for material iface nodes

///////////////OR ---

// generate new nodes on the back of BL

// replace nodes in orig hexes with back nodes

// check for mat iface nodes
...
void PostBL:: Algo2(){
  ...
  for(moab::Range::iterator nodes_iter = nodes.begin(); nodes_iter != nodes.end(); nodes_iter++){
   //    MBERRCHK(mb->tag_set_data(BLNodeIDTag, nodes, &matID[0]), mb);
  ...
  if (fixmat !=0 && (int) old_hex.size() > 1){
      ...
      std::cout << old_hex.size() << "#@$$@$@$@$" << std::endl;
    }
  ...
  std::cout<< "~~~~~~~~~~~~~2233232r" << surf_normal << std::endl;
  ...
  if(found == 1 && m_Material !=999){
      ...
      //TODO: THIS SHOULD CHANGE TO MAINTAIN ORIGINAL MATERIAL DEFINITION
  ...
  for (Range::iterator kter = nodes.begin(); kter != nodes.end(); ++kter){
      ...
      if(all_bl[count] == 0){
          ...
          std::cout << adj_for_normal.size() << " After intersection " << adj_for_norm.size() << std::endl;
          ...
          for(Range::iterator qter = adj_for_norm.begin(); qter != adj_for_norm.end(); ++qter){
              ...
              if(m_GD==3){
                  ...
                  if(sense == 1){
                      ...
                      std::cout << side_number << "sense " << sense << " surf_normal " << surf_normal << std::endl;
          ...
          // NODE B/W MATERIALS
          std::cout << " We're here in FIXMAT MM case, now go along the normal--- have fun in the process !!" << std::endl;
          moab::Range adj_for_normal;
          MBERRCHK(mb->get_adjacencies(&(*kter), 1, m_BLDim, false, adj_for_normal, Interface::UNION), mb);
          //create the coordinates of the innermost node corresponding to this node
          moab::Range adj_for_norm = intersect(quads, adj_for_normal);
          ...
          for(Range::iterator qter = adj_for_norm.begin(); qter != adj_for_norm.end(); ++qter){
              ...
              if(m_GD==3){
                  get_normal_quad (adj_qconn, v);
                  std::cout << side_number << "sense " << sense << "v " << v << std::endl;
          ...
          std::cout << edge_normal.size() << " Edge normal intersection " << adj_for_normal.size() << std::endl;

          if(edge_normal.size() > 1){
              std::cout << "MULTIPLE NORMALS ARE NEEDED" << std::endl;
            }
          else{
              std::cout << "We've one edge seperating materials 1 NORMAL IS NEEDED" << std::endl;
              //std::vector<moab::EntityHandle> edge_conn;
          ...
          std::cout << " Error, shouldn't have gotten here: " << count << std::endl;
      ...
      std::cout << " DEALING WITH COORD: " << coords_bl_quad[0]<< ","
                << coords_bl_quad[1]<< ","
                << coords_bl_quad[2]   << std::endl;
  ...
  for (Range::iterator kter = nodes.begin(); kter != nodes.end(); ++kter){
      ...
      if(all_bl[count] == 0 ){
          ...
          m_LogFile << std::setprecision (3) << std::scientific << " : NID:" << (nid)
                    ...
                    << ", " << coords_old_quad[1] << ", " << coords_old_quad[2] << " OLD:- coords: NEW" << coords_new_quad[0]
                    << ", " << coords_new_quad[1] << ", " << coords_new_quad[2]  << std::endl;
          //         std::cout << " We're here in MM case, now go along the edge --- have fun in the process !!" << std::endl;
