...
namespace MeshKit
{
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    std::vector<EntityHandle> conn(m_Intervals*8), qconn(4), adj_qconn(4),
      new_vert(m_Intervals*nodes.size()), old_hex, old_hex_conn(8), adj_hexes, adj_quads, adj_hex_nodes1(8);
	...
	for(int n=0; n< (int) nodes.size(); n++){
	  if(nodes[n] == qconn[i]){
	    ...
	    std::cout << "QUAD doesn't have a node in BL NODES, aborting.." << std::endl;
	  }
	}
	if (debug){
