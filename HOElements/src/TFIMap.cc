...
inline Point3D TFIMap::CoEdge::getXYZCoords(double ur)
{
     ...
     double u, umin, umax;
     geom->getEntURange(edgeHandle, umin, umax);
     assert(!err);
...
inline Point2D TFIMap::CoEdge::getUVCoords(double ur)
{
     ...
     double u, umin, umax;
     err = geom->getEntURange(edgeHandle, umin, umax);
     assert(!err);
...
{
     int err;
     vector<iBase_EntityHandle> faceedges;
     err = geom->getEntAdj(gFaceHandle, iBase_EDGE, faceedges);
     assert(!err);
...
//    iGeom_getEntAdj_Ext(geometry, gFaceHandle, iBase_VERTEX, facenodes, &err);
     ...
     for (int i = 0; i < 4; i++) {
          err = geom->getEntAdj(faceedges[i], iBase_VERTEX, edgenodes);
          assert(!err);
...
int TFIMap::reconstruct_structured_mesh2d()
{
     ...
     err = geom->getEntAdj(gFaceHandle, iBase_EDGE, gEdges);
...
//    iRel_getEntSetAssociation(assoc, rel, gFaceHandle, 0, &meshSet, &err);
     ...
     for (int i = 0; i < mFaces.size(); i++) {
          ...
          for (int j = 0; j < faceedges.size(); j++) {
               edgenodes.clear();
               err = mesh->getEntAdj(faceedges[j], iBase_VERTEX, edgenodes);
...
//  iRel_getEntSetAssociation(assoc, rel, coedge[0].edgeHandle, 0, &meshSet, &err);
     ...
     err = mesh->getEntAdj(mEdges[0], iBase_VERTEX, edgenodes);
     ...
     Point3D gxyz, mxyz;
     geom->getVtxCoord(gNodes[0], gxyz[0], gxyz[1], gxyz[2]);
     mesh->getVtxCoord(nodechain.front(), mxyz[0], mxyz[1], mxyz[2] );
     double d0 = square_length(gxyz, mxyz);

     geom->getVtxCoord(gNodes[1], gxyz[0], gxyz[1], gxyz[2]);
     mesh->getVtxCoord(nodechain.back(), mxyz[0], mxyz[1], mxyz[2]);
                        ...
                        iBase_EntityHandle n1, iBase_EntityHandle &edgeHandle)
{
     ...
     BOOST_FOREACH(edgeHandle, eset) {
          mesh->getEntAdj(edgeHandle, iBase_VERTEX, edgenodes);
...
//  iRel_getEntSetAssociation(assoc, rel, gFaceHandle, 0, &meshSet, &err);
     ...
     coedge[0].gface = gface;
     coedge[1].gface = gface;
     coedge[2].gface = gface;
     coedge[3].gface = gface;
...
void TFIMap::physicalTFI2D()
{
     ...
     for (int i = 0; i < nx * ny; i++)
          mesh->getVtxCoord(structured_nodes[i], x[0], y[0], z[0]);
     ...
     for (int j = 1; j < ny - 1; j++) {
          for (int i = 1; i < nx - 1; i++) {
               int id = j * nx + i;
               mesh->setVtxCoord(structured_nodes[id], x[id], y[id], z[id]);
          }
     }
}
...
void TFIMap::projectHigherOrderNodes2D(iBase_EntityHandle mFaceHandle, const vector<double> &glnodes)
{
     ...
     mesh->getData(mFaceHandle, horder_tag, &tag_val);
     assert(!err);
     ...
     for (int i = 0; i < numHPoints; i++)
          mesh->getVtxCoord(nodeHandles[i], x[i], y[i], z[i]);
     ...
     for (int i = 0; i < numHPoints; i++)
          mesh->setVtxCoord(nodeHandles[i], x[i], y[i], z[i] );
}
...
void TFIMap::projectHigherOrderNodes3D(iBase_EntityHandle mCellHandle, const vector<double> &glnodes)
{
     ...
     mesh->getData(mCellHandle, horder_tag, &tag_val);
     HO_Points *hopoints = (HO_Points *) tag_val;
     int nhx = hopoints->nx;
     int nhy = hopoints->ny;
     int nhz = hopoints->nz;
...
void TFIMap::saveAs(const string &filename)
{
     ...
     for (int i = 0; i < nx * ny; i++) {
          mesh->getVtxCoord(structured_nodes[i], x, y, z);
          assert(!err);
     ...
     vector<iBase_EntityHandle> gEdges;
     geom->getEntAdj(gFaceHandle, iBase_EDGE, gEdges);
...
//      iRel_getEntSetAssociation(assoc, rel, gEdges[k], 0, &meshSet, &err);
          ...
          for (int j = 0; j < mEdges.size(); j++) {
               ...
               iBase_EntityHandle *nodesOnEdge = hopoints->nodeHandles;
               for (int i = 1; i < numHPoints - 1; i++) {
                    mesh->getVtxCoord(nodesOnEdge[i], x, y, z);
                    ofile << node_index++ << " " << x << " " << y << " " << z << endl;
               }
          }
     }
...
//   iRel_getEntSetAssociation(assoc, rel, gFaceHandle, 0, &meshSet, &err);
     ...
     for (int i = 0; i < mEdges.size(); i++) {
          err = mesh->getData(mEdges[i], horder_tag, &tag_val);
          assert(!err);

          HO_Points *hopoints = (HO_Points *) tag_val;
          iBase_EntityHandle *nodesOnEdge = hopoints->nodeHandles;
          for (int j = 1; j < numHPoints - 1; j++) {
               mesh->getVtxCoord(nodesOnEdge[j], x, y, z);
               ofile << node_index++ << " " << x << " " << y << " " << z << endl;
          }
     }
     ...
     for (int k = 0; k < mFaces.size(); k++) {
          err = mesh->getData(mFaces[k], horder_tag, &tag_val);
          assert(!err);
          ...
          for (int j = 1; j < numHPoints - 1; j++) {
               for (int i = 1; i < numHPoints - 1; i++) {
                    mesh->getVtxCoord(nodesOnEdge[j * numHPoints + i], x, y, z);
                    ofile << node_index++ << " " << x << " " << y << " " << z << endl;
               }
          }
     }
