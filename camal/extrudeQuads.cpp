...
int main(int argc, char *argv[]) {
   ...
   if (argc < 4) {
      std::cout << "Usage: " << argv[0]
            << " <bottom_mesh> <top_filename> <out_mesh> [-n layers] "
  ...
  iMesh_getAdjEntIndices( mesh1, root_set,
                          iBase_FACE, iMesh_QUADRILATERAL, iBase_VERTEX,
                          &quads, &quads_alloc, &numQuads,
   ...
   iMesh_getVtxArrCoords(mesh1, vert_adj, vert_adj_size, iBase_INTERLEAVED, &xyz,
  ...
  // a next layer, and the hexas
   ...
   for (int i= 0; i<nbLayers; i++)
   {
       ...
       {
          for (int k=0; k<3; k++)
             xyz[3*j+k] += direction[k]*dArr[j]/nbLayers;
       }
