...
basic steps required to write a new meshing algorithm and use it with the rest of MeshKit.
...
instance stores a map between ModelEnt objects and moab::Range's operated on by the operation.

\section Declaration & Registration
Each new meshing operation should be derived from the MeshOp class (see e.g. VertexMesher for the proper syntax).  In addition,
the class should contain a static member variable, and that static member variable should be initialized with the output
of a call to MKCore::register_meshop.  Registering the new mesh operation with MKCore allows the algorithm to be requested
...
The MKCore::register_meshop function requires the following data passed as arguments:
 ...
 - The topological dimension(s) of ModelEnt's this meshing operation can operate on.
 - The MOAB entity type(s) of mesh entities produced by this meshing operation.
 - A pointer to a factory method on the meshing operation class that constructs instances of this meshing operation.  The
   factory method should be static, with the signature MeshOp* factory(MKCore *mkcore, const MEntVector &me_vec).
 - (Optional) A pointer to a function which evaluates whether the operation can operate on a specified ModelEnt; this function
   should have the signature bool canmesh(ModelEnt*), and should be static.

The MeshOp class provides several convenience functions that can be used in place of the options canmesh function.  These functions
...
\section setup_this and execute_this
...
\section Other Support or Convenience Functions
